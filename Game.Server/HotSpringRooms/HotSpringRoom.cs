    ...
    public class HotSpringRoom
    {
        ...
        public bool AddPlayer(GamePlayer player)
        {
            lock (_syncStop)
            {
                ...
                {
                    player.Out.SendMessage(eMessageType.Normal, LanguageMgr.GetTranslation("HotSpringRoom.Msg1"));
                    return false;
                }
        ...
        public void RemovePlayer(GamePlayer player)
        {
            lock (_syncStop)
            {
                ...
                //    GSPacketIn pkg = player.Out.SendPlayerLeaveHotSpringRoom(player);
                //    //0 player.CurrentHotSpringRoom.SendToPlayerExceptSelf(pkg, player);
                //    player.CurrentHotSpringRoom.SendToPlayerExceptSelfForScene(pkg, player);
        ...
        protected void OnTickForHymeneal(object obj)
        {
            try
            {
                ...
                //pkg.WriteByte((byte)HotSpringCmdType.HYMENEAL_STOP);
                ////0 SendToAll(pkg);
        ...
        public bool KickPlayerByUserID(GamePlayer player, int userID)
        {
            ...
            if (kickPlayer != null && kickPlayer.PlayerCharacter.ID != player.CurrentHotSpringRoom.Info.GroomID
                && kickPlayer.PlayerCharacter.ID != player.CurrentHotSpringRoom.Info.BrideID)
            {
        ...
        public void KickAllPlayer()
        {
            ...
            {
                RemovePlayer(p);
                p.Out.SendMessage(eMessageType.ChatNormal, LanguageMgr.GetTranslation("HotSpringRoom.TimeOver"));
            }
        }
        ...
        { 
            ...
            //{
            //    db.UpdateHotSpringRoomInfo(Info);
            //}
