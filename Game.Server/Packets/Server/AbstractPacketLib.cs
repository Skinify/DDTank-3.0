    ...
    public class AbstractPacketLib : IPacketLib
    {
        ...
        public static IPacketLib CreatePacketLibForVersion(int rawVersion, GameClient client)
        {
            ...
            while (true)
            {
                ...
                while (true)
                {
                    if (index < derivedClasses.Length)
                    {
                        type = derivedClasses[index];
                        customAttributes = type.GetCustomAttributes(typeof(PacketLibAttribute), false);
                        num2 = 0;
                    }
                    else
                    {
                ...
                while (true)
                {
                    ...
                    PacketLibAttribute attribute = (PacketLibAttribute)customAttributes[num2];
                    if (attribute.RawVersion == rawVersion)
                    {
                        try
                        {
                            object[] args = new object[] { client };
                            return (IPacketLib)Activator.CreateInstance(type, args);
                        }
        ...
        public GSPacketIn SendAuctionRefresh(AuctionInfo info, int auctionID, bool isExist, SqlDataProvider.Data.ItemInfo item)
        {
            ...
            if (isExist)
            {
                ...
                packet.WriteBoolean(!ReferenceEquals(item, null));
        ...
        public GSPacketIn SendBufferList(GamePlayer player, List<AbstractBuffer> infos)
        {
            ...
            using (List<AbstractBuffer>.Enumerator enumerator = infos.GetEnumerator())
            {
                while (true)
                {
                    if (!enumerator.MoveNext())
                    ...
                    AbstractBuffer current = enumerator.Current;
        ...
        public GSPacketIn SendFusionPreview(GamePlayer player, Dictionary<int, double> previewItemList, bool isbind, int MinValid)
        {
            ...
            using (Dictionary<int, double>.Enumerator enumerator = previewItemList.GetEnumerator())
            {
                while (true)
                {
                    if (!enumerator.MoveNext())
                    ...
                    KeyValuePair<int, double> current = enumerator.Current;
                    packet.WriteInt(current.Key);
        ...
        {
            if (!ReferenceEquals(this.m_gameClient.Player, null))
            {
                ...
                packet.WriteString("FFFFFFFFFFFFF");
        ...
        public GSPacketIn SendMarryRoomInfo(GamePlayer player, MarryRoom room)
        {
            ...
            bool val = !ReferenceEquals(room, null);
            packet.WriteBoolean(val);
            if (val)
            {
                packet.WriteInt(room.Info.ID);
        ...
        public GSPacketIn SendOpen(GamePlayer Player)
        {
            throw new NotImplementedException();
        }
        ...
        public GSPacketIn SendPlayerCardInfo(PlayerInfo player, List<SqlDataProvider.Data.ItemInfo> items)
        {
            ...
            using (List<SqlDataProvider.Data.ItemInfo>.Enumerator enumerator = items.GetEnumerator())
            {
                while (true)
                {
                    ...
                    SqlDataProvider.Data.ItemInfo current = enumerator.Current;
                    packet.WriteInt(current.Place);
                    if (items.Count <= 0)
                    {
                    ...
                    packet.WriteBoolean(true);
                    packet.WriteInt(current.ItemID);
                    packet.WriteInt(player.ID);
                    packet.WriteInt(current.Count);
                    packet.WriteInt(current.Place);
                    packet.WriteInt(current.Template.TemplateID);
                    packet.WriteInt(current.Attack);
                    packet.WriteInt(current.Defence);
                    packet.WriteInt(current.Agility);
                    packet.WriteInt(current.Luck);
                    packet.WriteInt(current.AttackCompose);
                    packet.WriteInt(current.DefendCompose);
                    packet.WriteInt(current.StrengthenLevel);
                    //packet.WriteInt(current.get_Hole5Exp());
                    //packet.WriteBoolean(current.get_IsGold());
                }
            }
            this.SendTCP(packet);
            return packet;
        ...
        public GSPacketIn SendRoomPlayerAdd(GamePlayer player)
        {
            ...
            bool val = false;
            if (!ReferenceEquals(player.CurrentRoom.Game, null))
            {
                val = true;
            }
            packet.WriteBoolean(val);
        ...
        public GSPacketIn SendUpdateBuffer(GamePlayer player, BufferInfo[] infos)
        {
            ...
            while (true)
            {
                ...
                BufferInfo info = infoArray[index];
                packet.WriteInt(info.Type);
                packet.WriteBoolean(info.IsExist);
                packet.WriteDateTime(info.BeginDate);
                packet.WriteInt(info.ValidDate);
        ...
        public void SendUpdateCardData(PlayerInventory bag)
        {
            if (bag.BagType == 15)
            {
                GSPacketIn packet = new GSPacketIn(0xd8);
                ...
                while (true)
                {
                    ...
                    packet.WriteInt(val);
                    if (ReferenceEquals(bag.GetItemAt(val), null))
                    {
                        packet.WriteBoolean(false);
                    }
                    else
                    {
                        packet.WriteBoolean(true);
                        SqlDataProvider.Data.ItemInfo itemAt = bag.GetItemAt(val);
                        packet.WriteInt(itemAt.ItemID);
                        packet.WriteInt(this.m_gameClient.Player.PlayerCharacter.ID);
                        packet.WriteInt(itemAt.Count);
                        packet.WriteInt(itemAt.Place);
                        packet.WriteInt(itemAt.Template.TemplateID);
                ...
                else
                {
                    ...
                    while (true)
                    {
                        ...
                        else
                        {
                            ...
                            packet.WriteInt(itemAt.Template.RefineryLevel);
                            packet.WriteDateTime(DateTime.Now.AddDays(5.0));
                            if (((itemAt.Template.CategoryID == 7) || (itemAt.Template.CategoryID == 1)) || (itemAt.Template.CategoryID == 5))
                            {
                                packet.WriteInt(9);
                                packet.WriteByte(5);
                                packet.WriteInt(9);
        ...
        {
            bool flag = !ReferenceEquals(this.m_gameClient.Player, null);
            if (flag)
            {
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        while (true)
                        {
                            ...
                            while (true)
                            {
                                flag = num5 < states.Length;
                                if (flag)
                                {
                                    packet.WriteByte(states[num5]);
                                    num5++;
                                    continue;
                                }
                                length -= val;
                ...
                {
                    Console.WriteLine(exception1.InnerException);
                }
            }
            else
            {
        ...
        public GSPacketIn SendUserEquip(PlayerInfo player, List<SqlDataProvider.Data.ItemInfo> items)
        {
            ...
            using (List<SqlDataProvider.Data.ItemInfo>.Enumerator enumerator = items.GetEnumerator())
            {
                while (true)
                {
                    ...
                    SqlDataProvider.Data.ItemInfo current = enumerator.Current;
                    packet.WriteByte((byte)current.BagType);
                    packet.WriteInt(current.UserID);
                    packet.WriteInt(current.ItemID);
                    packet.WriteInt(current.Count);
                    packet.WriteInt(current.Place);
                    packet.WriteInt(current.TemplateID);
                    packet.WriteInt(current.AttackCompose);
                    packet.WriteInt(current.DefendCompose);
                    packet.WriteInt(current.AgilityCompose);
                    packet.WriteInt(current.LuckCompose);
                    packet.WriteInt(current.StrengthenLevel);
                    packet.WriteBoolean(current.IsBinds);
                    packet.WriteBoolean(current.IsJudge);
                    packet.WriteDateTime(current.BeginDate);
                    packet.WriteInt(current.ValidDate);
                    packet.WriteString(current.Color);
                    packet.WriteString(current.Skin);
                    packet.WriteBoolean(current.IsUsed);
                    packet.WriteInt(current.Hole1);
                    packet.WriteInt(current.Hole2);
                    packet.WriteInt(current.Hole3);
                    packet.WriteInt(current.Hole4);
                    packet.WriteInt(current.Hole5);
                    packet.WriteInt(current.Hole6);
                    packet.WriteString(current.Template.Pic);
                    packet.WriteInt(current.Template.RefineryLevel);
                    packet.WriteDateTime(DateTime.Now);
                    packet.WriteByte(5);
                    packet.WriteInt(9);
                    packet.WriteByte(5);
